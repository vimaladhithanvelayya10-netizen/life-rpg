...
import { hashPassword, verifyPassword, createToken, requireAuth } from '../middleware/auth.js';
...
const getMailTransporter = () => {
  ...
  if (!user || !pass) {
    mailConfigError = 'SMTP credentials not configured. Using development OTP fallback.';
    return null;
  }
  ...
  try {
    ...
    if (!sent) {
      return res.status(502).json({
        error: 'Unable to send verification email. Please check your address or try again later.'
      });
    }
...
router.post('/verify-otp', async (req, res) => {
  try {
    ...
    if (!/^\d{4}$/.test(otp) || otp !== record.otp_code) {
      await query(`UPDATE auth_otps SET attempts = attempts + 1 WHERE id = $1`, [record.id]);
...
async function generateUniqueUsername(client, cleanName, birthDay, birthMonth) {
  ...
  while (true) {
    const res = await client.query(`SELECT 1 FROM users WHERE LOWER(username) = LOWER($1)`, [candidate]);
...
async function generateUniqueFriendCode(client) {
  while (true) {
    ...
    const res = await client.query(`SELECT 1 FROM user_profiles WHERE friend_code = $1`, [code]);
...
router.post('/create-account', async (req, res) => {
  try {
    const {
      email, onboardingToken, name, age, birthYear, birthDay, birthMonth,
      birthDate, contact, gender, password, profilePhoto
    } = req.body || {};
    ...
    if (!onboardingToken) throw new Error('Verify your email before creating an account.');
    if (cleanName.length < 2) throw new Error('Enter your full name.');
    if (!Number.isInteger(numAge) || numAge < 13 || numAge > 120) throw new Error('Enter a valid age (13-120).');
    if (!Number.isInteger(numBirthYear) || numBirthYear < 1900 || numBirthYear > new Date().getFullYear()) throw new Error('Enter a valid birth year.');
    if (!Number.isInteger(numBirthDay) || numBirthDay < 1 || numBirthDay > 31 || !Number.isInteger(numBirthMonth) || numBirthMonth < 1 || numBirthMonth > 12) {
      throw new Error('Enter a valid birth day and month.');
    }
    if (!['male', 'female'].includes(cleanGender)) throw new Error('Select whether your character is male or female.');
    ...
    if (!tokenRecord.rowCount) {
      return res.status(400).json({ error: 'Verification session invalid or expired. Please verify your email again.' });
    }

    // Save profile photo if provided
    let savedPhotoUrl = '';
    if (profilePhoto?.dataUrl) {
      const match = String(profilePhoto.dataUrl).match(/^data:(image\/(?:png|jpeg|jpg|webp));base64,(.+)$/i);
      if (!match) throw new Error('Profile photo must be PNG, JPG, JPEG, or WEBP.');
      const buffer = Buffer.from(match[2], 'base64');
      if (buffer.length > 5 * 1024 * 1024) throw new Error('Profile photo must be 5 MB or smaller.');
      await fs.mkdir(uploadsDir, { recursive: true });
      const ext = match[1].toLowerCase().replace('image/', '') === 'jpeg' ? 'jpg' : match[1].toLowerCase().replace('image/', '');
    ...
    const newUser = await tx(async (client) => {
      // Check if email already registered
      const emailCheck = await client.query('SELECT 1 FROM users WHERE LOWER(email) = $1', [normalizedEmail]);
      if (emailCheck.rowCount > 0) throw Object.assign(new Error('An account with this email already exists.'), { status: 409 });

      const username = await generateUniqueUsername(client, cleanName, numBirthDay, numBirthMonth);
      ...
      `, [
        user.id, friendCode, cleanGender, birthDate || null, numBirthYear,
        numBirthMonth, numBirthDay, numAge, cleanGender, cleanContact, savedPhotoUrl || null
      ]);
    ...
    const tokenHash = crypto.createHash('sha256').update(token + crypto.randomUUID()).digest('hex');
    ...
    return res.status(err.status || 400).json({ error: err.message || 'Could not create account.' });
  }
});
...
router.post('/login', async (req, res) => {
  try {
    const identity = String(req.body?.identity || req.body?.login || req.body?.username || req.body?.email || '').trim().toLowerCase();
    ...
    if (!userRes.rowCount) {
      return res.status(401).json({ error: 'We could not find an account with that username or email.' });
    }
    ...
    await query(`
      ...
      ON CONFLICT (refresh_token_hash) DO NOTHING
    `, [user.id, token, tokenHash, req.ip || null, req.header('user-agent') || null]);
...
router.delete('/account', requireAuth, async (req, res) => {
  try {
    ...
    const userRes = await query(`SELECT password_hash FROM users WHERE id = $1::uuid`, [req.userId]);
    if (!userRes.rowCount) {
    ...
    await tx(async (client) => {
      ...
      await client.query(`DELETE FROM auth_sessions WHERE user_id = $1::uuid`, [req.userId]);
    ...
    return res.status(200).json({ ok: true, message: 'Your LIFE RPG account has been completely deleted.' });
  } catch (err) {
